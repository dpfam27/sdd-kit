...
function loadContext(projectRoot: string, needs: string[]): string {
  const parts: string[] = [];
...
function openSpecDocs() {
  ...
  vscode.window.showQuickPick(items.map((i) => i.label)).then((picked: string | undefined) => {
    if (picked !== undefined) {
      const item = items.find((i) => i.label === picked);
      if (item) vscode.window.showTextDocument(vscode.Uri.file(item.path));
    }
  });
}
...
<script>
  ...
  private _loadDocs(root: string): Array<{ name: string; path: string; size: string }> {
    const docsDir = path.join(root, "docs");
